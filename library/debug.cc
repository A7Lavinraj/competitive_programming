...
void DBG(const pair<T, V> &x);
template<typename T>
void DBG(const T &x) {int f = 0; cerr << '{'; for (auto &i: x) cerr << (f++ ? ", " : ""), DBG(i); cerr << "}";}
template<typename T, typename V>
void DBG(const pair<T, V> &x) {cerr << '{'; DBG(x.first); cerr << ", "; DBG(x.second); cerr << '}';}
void dbg() {cerr << "]\n";}
template <typename T, typename... V>
